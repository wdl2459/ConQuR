...
ConQuR_each <- function(y, X, X_span, X_correct, X_span_correct,
  ...
  sy = 1*(y > 0)
  if ( sum(sy) == 1 ){ y_new = y; return(y_new) } # keep original count when there is only 1 meaningful observation
  ...
  if ( sum( table(sy) <= logistic_thres ) | length(table(sy)) == 1 | logistic_lasso == F ){
    ...
    batch_null_values = apply( X_logistic_correct[, grep( "batchid", colnames(X_logistic_correct) )], 2, min )
  ...
  if (quantile_type == "standard"){
    ...
    batch_null_values = apply( X_quantile_correct[, grep( "batchid", colnames(X_quantile_correct) )], 2, min )
